...
# and run `turbo prune`. This is more efficient than copying the entire repository.
FROM base as pruner
...
# The APP_NAME is passed in as a build argument.
ARG APP_NAME
...
# Copy the pruned package manifests and the text-based lockfile generated by prune.
COPY --from=pruner /app/out/json/ ./
COPY --from=pruner /app/out/bun.lock ./bun.lockb
